...
const isInGroupMiddleWare = () => async (ctx: TelegrafContext, next: NextFunction) => {
  try {
    ...
    logger.info(NAMESPACE, `chat: ${chat.id} chat_name: ${chat.id}, text: ${text}`);
    ...
    if (chat.type === 'private' && !allowedCommands.includes(text)) {
      return await ctx.reply('Сначала добавь меня в чат, эти комманды не для приватных диалогов');
    }
    if ((chat.type === 'supergroup' || chat.type === 'group') && isChatCommand) {
      const chatExists = await getChatByChatId(chat.id);
      if (!chatExists) {
        return await ctx.reply('похоже вы не нажали /start после добавления бота в группу');
      }
    }
    return next();
  } catch (error) {
